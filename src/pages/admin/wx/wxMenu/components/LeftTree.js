...
export default class LeftTree extends React.Component {
  ...
  render() {
    ...
    const menus = treeData.map((obj) => {
      return (
        <TreeNode title={buildTitle(obj.menu.name + "("+ obj.children.length +")")} key={`root_${obj.menu.id}`} >
          {
            obj.children.map((sub)=> {return (<TreeNode title={buildTitle(sub.name)} key={`child_${sub.id}`}>
